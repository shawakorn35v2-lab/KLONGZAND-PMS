...
- ✅ `/inventory` — 5 แท็บ: **สต๊อก** (ของใช้ภายใน is_for_sale=false/null เท่านั้น, admin เพิ่ม/ตั้งค่า), **รับ/เบิก** (stock_in/stock_out ของใช้ภายใน, dropdown กรอง non-sale items), **ขายของ** (สินค้าขาย is_for_sale=true แยกชัดเจน — "+ เพิ่มสินค้าขาย" เช็คชื่อซ้ำก่อน insert: ถ้าชื่อซ้ำแต่ยังไม่ใช่ sale item จะ popup ถามยืนยัน "ตั้งให้ของเดิมเป็นสินค้าขาย" แทน, ถ้าซ้ำและเป็น sale item แล้วแจ้งเตือนสั้นๆ; ปุ่ม "รับเข้า" per-row เพิ่มสต๊อก, ปุ่ม "ขาย" per-row บันทึก transaction income + movement sale atomic), **คำขอ** (inventory_requests, admin อนุมัติ/ปฏิเสธ), **ประวัติ** (ทุกประเภท, badge สีเข้ม 3 สี: 📦 รับเข้า/📤 เบิกออก/🛒 ขาย); กันเกินสต๊อก client+server; rollback transaction ถ้า movement insert ล้มเหลว
