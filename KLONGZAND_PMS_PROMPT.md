...
- ✅ **โลโก้ระบบใช้รูปจริง** (แทน emoji 🏖️ เดิม): `public/logo-icon.png` (ไอคอนเดี่ยว ไม่มีตัวหนังสือ) + `public/logo-full.png` (โลโก้เต็มมี "KLONGZAND PMS" + "PROPERTY MANAGEMENT SYSTEM") — favicon ตั้งใน `metadata.icons.icon = '/logo-icon.png'` ของ `app/layout.js` (Next.js 15 App Router metadata API สร้าง `<link rel="icon">` อัตโนมัติ); หน้า login (`app/login/page.js`) ใช้ `<Image src="/logo-full.png" priority>` (next/image) แทน emoji+h1+subtitle เพราะข้อความอยู่ในรูปแล้ว; sidebar header (`components/Sidebar.js`) ใช้ `<Image src="/logo-icon.png" width={40} height={40}>` คู่กับข้อความ "KLONGZAND / PMS"; mobile topbar (`components/LayoutShell.js`) ใช้ `<Image width={28} height={28}>` คู่กับข้อความ "KLONGZAND PMS"; ทุกจุดใช้ `next/image` เพื่อ optimize เป็น webp อัตโนมัติ